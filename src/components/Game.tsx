...
function spawnPegs() {
	...
	pins.forEach((pin) => {
		pegs.push(
			Bodies.circle(pin.position.x + pin.size / 2, pin.position.y + pin.size / 2, pin.size / 2, {
				isStatic: true,
				restitution: 0.6,
				friction: 0.05,
				id: pin.id,
...
function createFloors() {
	const { FLOOR_WIDTH, FLOOR_HEIGHT, PIN_VERTICAL_GAP, PINS_LEVEL_COUNT, PIN_START_Y, FLOOR_VERTICAL_GAP } =
		GAME;
...
export default function Game() {
	...
	useEffect(() => {
		...
		const initializeObjects = () => {
			...
			World.add(world, [...createBoundaries(), ...spawnPegs().pegs, ...floorsBodies]);
		};

		// 🔹 Handle Collisions
		const handleCollisions = (event: Matter.IEventCollision<Matter.Engine>) => {
			const collidedBodyIds = new Set(event.pairs.flatMap(({ bodyA, bodyB }) => [bodyA.id, bodyB.id]));
			...
			event.pairs.forEach(({ bodyA, bodyB }) => {
				...
				const floor = floorsRef.current.find((f) => f.id === bodyA.id || f.id === bodyB.id);
		...
		const spawnBalls = (event: Matter.IEventTimestamped<Matter.Engine>) => {
			...
			if (now - lastSpawnTime.current >= spawnDelay.current) {
				...
				spawnDelay.current = Math.max(spawnDelay.current * 0.9, 50);
			}
		};
